...
`
    ...
    1. Реализация burger menu на обеих страницах: +24
        ...
        * бургер-иконка создана при помощи html+css, без использования изображений: +0
    ...
    2. Реализация слайдера-карусели на странице Main: +33
        ...
        * при изменении ширины экрана (от 1280px до 320px и обратно), слайдер перестраивается и работает без перезагрузки страницы (набор карточек при этом может как изменяться, так и оставаться тем же, скрывая лишнюю или добавляя недостающую, и сохраняя при этом описанные для слайдера требования): [+1] (не всегда хорошо подстраивается, нужно иногда перезагружать страницу)
    3. Реализация пагинации на странице Pets: (14) (прокрутка работает (не очень хорошо) только ">")
        * при перезагрузке страницы всегда открывается первая страница пагинации: +2
        * при загрузке страницы формируется массив из 48 объектов питомцев. Каждый из 8 питомцев должен встречаться ровно 6 раз: +4 (вывожу массив в консоль, можете проверить неповторение карточек там)
